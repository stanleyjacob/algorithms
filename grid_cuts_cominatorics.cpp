...
int ways(vector<string>& grid, int k) {
    vector<vector<vector<int> > > dp(k, vector<vector<int> >(grid.size(), vector<int>(grid[0].size(), 0)));
    ...
    vector<vector<int> > appleNum(grid.size() + 1, vector<int>(grid[0].size() + 1, 0));
    for (int i = grid.size() - 1; i >= 0; --i) {
        for (int j = grid[0].size() - 1; j >= 0; --j) {
            appleNum[i][j] = (int)(grid[i][j] == 'A') + appleNum[i + 1][j] + appleNum[i][j + 1] - appleNum[i + 1][j + 1];
        }
    }
